...
object KoreanSubstantive {
  ...
  protected[processor] def isJosaAttachable(prevChar: Char, headChar: Char): Boolean = {
  ...
  protected[processor] def isKoreanNumber(chunk: CharSequence): Boolean =
   ...
   */
  protected[processor] def collapseNouns(posNodes: Seq[KoreanToken]): Seq[KoreanToken] = {
